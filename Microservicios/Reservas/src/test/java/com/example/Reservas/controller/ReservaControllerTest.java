...
class ReservaControllerTest {
    ...
    @Test
    void testConfirmarReserva_NoExiste() throws Exception {
        // Arrange
        when(reservaService.confirmarReserva(999L))
                .thenThrow(new RuntimeException("Reserva no encontrada con id: 999"));

        // Act & Assert
        mockMvc.perform(patch("/api/reservas/999/confirmar"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").exists());

        verify(reservaService, times(1)).confirmarReserva(999L);
    }
    ...
    @Test
    void testCancelarReserva_NoExiste() throws Exception {
        // Arrange
        when(reservaService.cancelarReserva(eq(999L), anyString()))
                .thenThrow(new RuntimeException("Reserva no encontrada con id: 999"));

        // Act & Assert
        mockMvc.perform(patch("/api/reservas/999/cancelar")
                        .param("motivo", "Motivo"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").exists());

        verify(reservaService, times(1)).cancelarReserva(eq(999L), anyString());
    }
    ...
    @Test
    void testCompletarReserva_NoExiste() throws Exception {
        // Arrange
        when(reservaService.completarReserva(999L))
                .thenThrow(new RuntimeException("Reserva no encontrada con id: 999"));

        // Act & Assert
        mockMvc.perform(patch("/api/reservas/999/completar"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").exists());

        verify(reservaService, times(1)).completarReserva(999L);
    }
    ...
    @Test
    void testActualizarReserva_NoExiste() throws Exception {
        // Arrange
        ...
        when(reservaService.actualizarReserva(eq(999L), any(Reserva.class)))
                .thenThrow(new RuntimeException("Reserva no encontrada con id: 999"));
        ...
        verify(reservaService, times(1)).actualizarReserva(eq(999L), any(Reserva.class));
    }
}
