...
class ReservaServiceTest {
    ...
    void testActualizarReserva_Exitoso() {
        ...
        verify(reservaRepository, times(1)).findById(1L);
        verify(reservaRepository, times(1)).save(any(Reserva.class));
        // No se verifica disponibilidad cuando las fechas no cambian
        verify(reservaRepository, never()).findReservasSolapadas(anyLong(), any(), any());
    }

    @Test
    void testActualizarReserva_ConCambioDeFechas_Disponible() {
        // Arrange
        LocalDateTime nuevaFechaInicio = LocalDateTime.of(2024, 1, 16, 10, 0);
        LocalDateTime nuevaFechaFin = LocalDateTime.of(2024, 1, 16, 12, 0);
        
        Reserva reservaActualizada = new Reserva();
        reservaActualizada.setUsuarioId(1L);
        reservaActualizada.setCanchaId(1L);
        reservaActualizada.setFechaInicio(nuevaFechaInicio);
        reservaActualizada.setFechaFin(nuevaFechaFin);
        reservaActualizada.setPrecioTotal(new BigDecimal("120000"));
        reservaActualizada.setEstado(Reserva.EstadoReserva.CONFIRMADA);

        when(reservaRepository.findById(1L)).thenReturn(Optional.of(reserva));
        when(reservaRepository.findReservasSolapadas(1L, nuevaFechaInicio, nuevaFechaFin))
                .thenReturn(Collections.emptyList());
        when(reservaRepository.save(any(Reserva.class))).thenReturn(reserva);
        ...
        assertNotNull(resultado);
        verify(reservaRepository, times(1)).findById(1L);
        verify(reservaRepository, times(1)).findReservasSolapadas(1L, nuevaFechaInicio, nuevaFechaFin);
        verify(reservaRepository, times(1)).save(any(Reserva.class));
    }

    @Test
    void testActualizarReserva_ConCambioDeFechas_NoDisponible() {
        // Arrange
        LocalDateTime nuevaFechaInicio = LocalDateTime.of(2024, 1, 16, 10, 0);
        LocalDateTime nuevaFechaFin = LocalDateTime.of(2024, 1, 16, 12, 0);
        ...
        reservaSolapada.setId(2L);

        when(reservaRepository.findById(1L)).thenReturn(Optional.of(reserva));
        when(reservaRepository.findReservasSolapadas(1L, nuevaFechaInicio, nuevaFechaFin))
                .thenReturn(Arrays.asList(reservaSolapada));

        // Act & Assert
        assertThrows(RuntimeException.class, () -> {
            reservaService.actualizarReserva(1L, reservaActualizada);
        });
        verify(reservaRepository, times(1)).findById(1L);
        verify(reservaRepository, times(1)).findReservasSolapadas(1L, nuevaFechaInicio, nuevaFechaFin);
        verify(reservaRepository, never()).save(any(Reserva.class));
